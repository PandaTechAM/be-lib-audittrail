...
public abstract class AuditTrailServiceBase<TPermission> : IAuditTrailService<TPermission>, IDisposable,
   IAsyncDisposable
{
   ...
   private readonly ConcurrentBag<AuditTrailDataBeforeSave<TPermission>> _auditTrailSaveData = [];

   private readonly ConcurrentBag<AuditTrailDataAfterSave<TPermission>> _auditTransactionData = [];
   ...
   protected AuditTrailServiceBase(IAuditTrailConsumer<TPermission> auditTrailConsumer,
      ...
      CancellationToken cancellationToken = default)
   {
      ...
      foreach (var entityEntry in changes.Where(s => s?.Entity != null))
      {
         ...
         var auditData = new AuditTrailDataBeforeSave<TPermission>
         {
            ...
            EntityId = id,
            EntityName = entityEntry.GetType()
                                    .GetCustomAttribute<DisplayNameAttribute>()
                                    ?.DisplayName ?? entityEntry.Entity.GetType()
                                                                .Name,
            Action = GetActionFromEntityState(entityEntry.State),
            DataJson = JsonSerializer.Serialize(entityProperties.TrackedProperties),
   ...
   public async Task TransactionFinished(TransactionEventData eventData,
   ...
   public async Task FinishSaveChanges(DbContextEventData eventData, CancellationToken cancellationToken = default)
   {
      if (eventData.Context != null && !_commitStarted)
      {
         ...
         {
            await SendToConsumerAsync(updatedData, CancellationToken.None);
         }
         else
         {
   ...
   public async Task SavingChangesStartedAsync(DbContextEventData eventData,
      CancellationToken cancellationToken = default)
   {
      if (_commitStarted)
      {
   ...
   public Task BeforeTransactionCommitedAsync(DbTransaction transaction,
      TransactionEventData eventData,
      CancellationToken cancellationToken = default)
   {
      if (eventData.Context?.Database.CurrentTransaction != null
          && _auditTransactionData.Any(s =>
             (s.Action == AuditActionType.Update && s.ModifiedProperties.Any()) || s.Action != AuditActionType.Update))
      {
      ...
      object entity)
   {
      ...
      if (ruleService == null)
      {
         throw new ArgumentNullException($"Missing service for rule {entity.GetType().FullName}");
      }
      ...
      CancellationToken cancellationToken = default)
   {
      try
      {
         if (auditTrailData.Any(s =>
                (s.Action == AuditActionType.Update && s.ModifiedProperties.Any()) ||
                s.Action != AuditActionType.Update))
         {
