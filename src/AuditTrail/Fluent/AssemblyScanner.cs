...
public class AssemblyScanner : IEnumerable<AssemblyScanner.AssemblyScanResult>
{
   ...
   ///    Whether to include internal types in the search as well as public types. The default
   ///    is false.
   /// </param>
   ...
   public static AssemblyScanner FindTypeInAssemblies(IEnumerable<Assembly> assemblies,
      bool includeInternalTypes = false)
   {
      var types = assemblies.SelectMany(x => includeInternalTypes ? x.GetTypes() : x.GetExportedTypes())
                            .Distinct();
   ...
   {
      var query = GetTypes(typeof(IEntityRule<,>))
         .ToList();
